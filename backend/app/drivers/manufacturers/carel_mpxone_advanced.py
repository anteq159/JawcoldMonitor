...
@register_driver("Carel MPXone Advanced")
class CarelMpxOneAdvancedDriver(AbstractControllerDriver):
    """Carel MPXone, Advanced configuration tier - adds direct EEV valve
    positioning on top of the Medium tier's EEV/HACCP feature set.
    Register addresses/types are real, taken from Carel's own "MPXone
    MODBUS Variable List.xlsx" (MPXone_ADVANCED sheet) - see the Basic
    tier's docstring for the full caveat on data type/scale inference,
    including the note on "FOk"'s real name/numbering."""

    manufacturer = "Carel MPXone Advanced"
    ...
    def default_register_map(self) -> List[RegisterMapEntry]:
        return [
            ...
            RegisterMapEntry(address=155, name="Ręczne położenie zaworu (PMu)", unit="kroki", data_type="uint16", writable=True, register_type="holding"),
    ...
    def identify(self, model_hint: Optional[str] = None) -> ControllerModel:
        return ControllerModel(model=model_hint or "MPXone Advanced", description="Carel MPXone (konfiguracja Advanced) z ręcznym pozycjonowaniem zaworu EEV")
    ...
    def simulate_reading(self, tick: float) -> Dict[str, dict]:
        supply = round(0.5 + 1.2 * math.sin(tick * 0.06) + random.uniform(-0.2, 0.2), 1)
        ...
        valve_steps = round(max(0, min(480, 200 + 40 * math.sin(tick * 0.09) + random.uniform(-10, 10))), 0)
        return {
            "Sonda Sm (nawiew)": {"value": supply, "unit": "°C"},
            "Sonda Sr (powrót)": {"value": return_air, "unit": "°C"},
            "Nastawa (St)": {"value": 0.5, "unit": "°C"},
            "Różnica załączania (rd)": {"value": 2.0, "unit": "°C"},
            "Nastawa przegrzania (P3)": {"value": 6.0, "unit": "K"},
            "Ręczne położenie zaworu (PMu)": {"value": valve_steps, "unit": "kroki"},
            ...
            "Zawór elektromagnetyczny (DO_sol)": {"value": 1 if supply > 0.5 else 0, "unit": ""},
            "Odszranianie (DO_def)": {"value": 0, "unit": ""},
            "Wentylator (DO_Fan)": {"value": 1, "unit": ""},
            "Sprężarka 2 (FOk)": {"value": 1 if supply > 0.5 else 0, "unit": ""},
