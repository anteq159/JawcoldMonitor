...
class EliwellDriver(AbstractControllerDriver):
    """Representative profile for an Eliwell ID961/IDPlus-series commercial
    refrigeration cabinet controller. Demonstration register map for the
    Etap 1 simulation layer - verify against the specific model's official
    Modbus map before use against real hardware."""
    ...
    def identify(self, model_hint: Optional[str] = None) -> ControllerModel:
        return ControllerModel(model=model_hint or "IDPlus 974", description="Sterownik chłodniczy Eliwell IDPlus")
